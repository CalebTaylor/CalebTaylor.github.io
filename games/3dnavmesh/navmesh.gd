...
func _process(delta):
	if (path.size() > 1):
		...
		var t = Transform()
		t.origin = atpos
		#t=t.looking_at(atpos + atdir, Vector3(0, 1, 0))ar position3D = dropPlane.intersects_ray(camera.project_ray_origin(get_viewport().get_mouse_position()), camera.project_ray_normal(get_viewport().get_mouse_position()))
		t=t.looking_at(Vector3(get_viewport().get_mouse_position().x, 1, 0), Vector3(0, 1, 0))
		#t=t.looking_at(Vector3(a, 1, 0), Vector3(0, 1, 0))
...
#	if (event extends InputEventMouseButton and event.button_index == BUTTON_LEFT and event.pressed):
	if (event.is_class("InputEventMouseButton") and event.button_index == BUTTON_LEFT and event.pressed):
		var from = get_node("robot_base/Camera").project_ray_origin(event.position)
		var to = from + get_node("robot_base/Camera").project_ray_normal(event.position)*100
